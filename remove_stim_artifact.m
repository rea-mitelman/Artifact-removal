function clean_signal=remove_stim_artifact(...
    raw_signal, raw_signal_Fs, stim_times, org_art_dur, dead_time, nan_flag, do_lin_decay)
%
% clean_signal=remove_stim_artifact(raw_signal, ...
% raw_signal_Fs, stim_times, org_art_dur, dead_time, nan_flag,
% do_lin_decay)
% Cleans the raw signal from the stimulus artifact, by zeroing the dead
% time (defined under the two entry vector "dead_time", time-begin till     
% time-end of saturated period, in ms, which should be "erased");averaging
% and subtraction of the average artifact in a decay manner: from
% t=dead_time(2) till t=org_art_dur the value is fully subtracted; from
% t=org_art_dur till t=art_dur=2*org_art_dur, the subtraction value is
% decayed linearly to zero (i.e., at t=org_art_dur there is a full
% subtraction of the average artifact, whereas at t=art_dur there is no
% subtraction de-facto. 
% Inputs:
